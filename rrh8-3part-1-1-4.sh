...
echo -e "\n# 1.1.4 Ensure nosuid option set on /tmp partition : [FAILED]" | tee -a $LOG

# Check for the following output
r1_1_4=$(/usr/bin/mount | /usr/bin/grep 'on /tmp ')
echo 'Set Variable: $r1.1.4="'$r1_1_4'"' | tee -a $LOG

if [[ $r1_1_4 == "" ]] 
then
    echo -e "1.1.4" $REXEC | tee -a $LOG
    ...
    sed -i "s/^tmpfs\t\/tmp\ttmpfs\tdefaults,rw.*/tmpfs\t\/tmp\ttmpfs\tdefaults,rw,nosuid,nodev,noexec,relatime\t0\t0/w /root/changelog.txt" "/etc/fstab"
    ...
    # After remediation
    # echo "unset dr1_1_4 for reuse.." | tee -a $LOG
    # unset dr1_1_4
    echo 'Show Variable: $dr1.1.4="'$dr1_1_4'"' | tee -a $LOG
    dr1_1_4=$(/usr/bin/mount | /usr/bin/grep 'on /tmp ')
    
    if [[ $dr1_1_4 == "[\s]*[,]?nosuid" ]]
    then
        echo $EXPECTED $dr1_1_4 | tee -a $LOG
        echo $RAPP | tee -a $LOG
    else
        echo -e "\n!!! REMEDIATION IS NOT SUCCESSFUL FOR # 1.1.4\n" | tee -a $LOG
        my_array+=("!!! REMEDIATION IS NOT SUCCESSFUL FOR # 1.1.4")
        # echo -e '\n$counter: '$counter"\n"
        # echo -e "${my_array[3]}"
        # ((counter++))
        echo 'Output: "'$dr1_1_4'"' | tee -a $LOG
    fi
else
    echo 'Show Variable: $r1.1.4="'$r1_1_4'"' | tee -a $LOG
    ...
    echo '$diffstab:"'$diffstab'"' | tee -a $LOG
